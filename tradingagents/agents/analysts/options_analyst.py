...
For CSP, focus on conditions favorable to the put seller: durable bullish/neutral thesis, acceptable assignment price, adequate cash collateral, rich but not crisis-distorted IV, favorable premium relative to downside risk, short-put Delta and probability trade-offs, Theta decay, liquidity, expected move, earnings/events, and whether the investor would be happy owning the shares. Evaluate the approximately 45-DTE window and ATM/near-ATM contract, while recognizing that ATM CSP has substantial downside and assignment exposure.

Also construct a separate OTM income candidate for an investor whose objective is premium collection with no shares. Prefer a defined-risk OTM put credit spread rather than an uncovered short put: sell the OTM put with Delta initially in the 0.10-0.20 range and buy a farther OTM protective put. Evaluate whether that Delta band is stable using expected move, IV/skew, Gamma, catalysts, liquidity, spread width, credit, max loss, breakeven, and return on defined risk. If 0.10-0.20 is not stable, select a lower-Delta short leg instead. Require closing the spread before expiration and state that Delta alone cannot guarantee zero assignment risk.

For LEAPS Call, evaluate the buyer's premium-at-risk, long-dated Delta, Vega, Theta, IV percentile/term structure, breakeven, liquidity, catalyst, and the risk that time value or IV falls even if the stock thesis is right. Retrieve/analyze chains near 45 DTE, near 365 DTE, and an OTM spread candidate when available.
