...
After deciding whether CSP is suitable, evaluate the knowledge-graph strategies supported by the available chain and select the single best non-CSP strategy only when its own entry conditions and risk profile are supported by current data. Do not force an alternative: if no strategy qualifies, say so. LEAPS is one optional candidate, not the default alternative.
